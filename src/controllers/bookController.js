...
const getBooksInYear = async function(req,res){
    ...
    let allBooks = await BookModel.find({ year: {$eq: bookYear}})
    res.send({msg: allBooks})
}
...
const getXINRBooks = async function(req,res){
    let allBooks = await BookModel.find({$or:[{"prices.indianPrice":"100"},{"prices.indianPrice":"200"},{"prices.indianPrice":"500"}]})
    res.send({msg: allBooks})
}

const getRandomBooks = async function(req,res){
    let allBooks = await BookModel.find({$in:[{stockAvailable:true},{totalPages:{$gt:500}}]})
    res.send({msg: allBooks})
}
